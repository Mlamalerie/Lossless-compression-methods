...
class Huffman[S](source : Seq[S]) extends StatisticCompressor[S](source)
  {
    ...
    def get_two_lowest_to(nodes_list: Seq[EncodingTree[S]]) = {
      nodes_list.sortWith(_.label > _.label).takeRight(2).toList
    }
    def create_one_node(g : EncodingTree[S],d: EncodingTree[S])  = {
      val value = g.label + d.label
      EncodingNode(value,g,d)
    }
    def delete_one_node_to(drop : EncodingTree[S],nodes_list : List[EncodingTree[S]])  = {
      nodes_list.filter( _ != drop)
    }
    def delete_nodes_to(drops : List[EncodingTree[S]],nodes_list : List[EncodingTree[S]])  = {
      nodes_list.filter( !drops.contains(_) )
    }

    def add_one_node_to(adding: EncodingTree[S],nodes_list : List[EncodingTree[S]])  = {
      adding +: nodes_list
    }
    ...
    lazy val tree : Option[EncodingTree[S]] =
    {
      ...
      leafsOrdered match {
        case leafs : List[EncodingTree[S]] => {
          leafs match {
