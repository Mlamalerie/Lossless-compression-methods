...
class LZW(val initialDictionary: Dictionary = ASCII) extends Compressor[Char, Seq[Int]] {
	...
	def compress(msg: Seq[Char]): Seq[Int] = {
		...
		@tailrec
		def lecture(msg_total: Seq[Char], dict: Dictionary, encoded_result: Seq[Int] = Seq(), cursor_l: Int = 0, cursor_r: Int = 1): Seq[Int] = {
			...
			if (cursor_r < msg_total.length) {
				dict.indexOf(select_x_str) match {
					...
					case i: Int if i == -1 => {
						...
						//println("add", add)
						lecture(msg_total, new_dict, encoded_result :+ output, cursor_l + add, cursor_r + 1)
					}
	...
	def uncompress(res: Seq[Int]): Option[Seq[Char]] = {
		@tailrec
		def lecture(msg_received_total: Seq[Int], dict: Dictionary, decoded_result_total: Seq[String] = Seq(), cursor_msg: Int = 0): Seq[Char] = {
			...
			if (cursor_msg < msg_received_total.length) {
				...
				if (decoded_result_total.nonEmpty) {
					...
					lecture(msg_received_total, new_dict, decoded_result_total :+ d_res, cursor_msg + 1)
				}
				// si est au début du processus
				else {
					println("\n@ début du processus !")
					lecture(msg_received_total, dict, decoded_result_total :+ dict(x_int_received), cursor_msg + 1)
				}
			} else {
				println(" THE END")
